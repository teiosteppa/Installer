...
extern "system" fn dlg_proc(dialog: HWND, message: u32, wparam: WPARAM, lparam: LPARAM) -> isize {
    match message {
        WM_INITDIALOG => {
        ...
        WM_COMMAND => {
            ...
            match control_id {
                IDC_LANGUAGE_COMBO if ncode == CBN_SELCHANGE => {
                    update_target(dialog, unsafe { GetDlgItem(dialog, IDC_TARGET).unwrap() },
